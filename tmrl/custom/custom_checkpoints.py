...
def update_run_instance(run_instance, training_cls):
    ...
    assert ALG_NAME in ["SAC", "REDQSAC", "TQC", "DroQ"], f"{ALG_NAME} is not supported by this checkpoint updater."

    if ALG_NAME in ["SAC", "REDQSAC", "TQC", "DroQ"]:
        ...
        if ALG_NAME == "DroQ":
            dropout_rate = ALG_CONFIG["DROQ_DROPOUT_RATE"]
            n_dropout_samples = ALG_CONFIG["DROQ_N_DROPOUT_SAMPLES"]

            if run_instance.agent.dropout_rate != dropout_rate:
                old = run_instance.agent.dropout_rate
                run_instance.agent.dropout_rate = dropout_rate
                for m in run_instance.agent.model.modules():
                    if isinstance(m, torch.nn.Dropout):
                        m.p = dropout_rate
                logging.info(f"Dropout rate switched to {dropout_rate} (old: {old}).")

            if run_instance.agent.n_dropout_samples != n_dropout_samples:
                old = run_instance.agent.n_dropout_samples
                run_instance.agent.n_dropout_samples = n_dropout_samples
                logging.info(f"Number of dropout samples switched to {n_dropout_samples} (old: {old}).")
